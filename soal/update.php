<?php
    ...
    if(strlen($_POST["kode_soal"])==0) {
        ...
        $data_soal = $db->getFieldValue("soal_head AS A LEFT JOIN mapel AS B ON A.id_pelajaran=B.id", array("COUNT(*) AS TOTAL"), array("A.id_kelas" => $_POST["id_kelas"], "A.id_pelajaran" => $_POST["id_pelajaran"], "A.kode_soal" => $_POST["kode_soal"]), array(), 0, 0, "", "A.id<>'$id'");
    ...
    $data_waktu = $db->getFieldValue("soal_head AS A LEFT JOIN mapel AS B ON A.id_pelajaran=B.id", array("COUNT(*) AS TOTAL"), array("B.id_kelas" => $_POST["id_kelas"]), array(), 0, 0, "", " A.tanggal_jam BETWEEN '".$_POST["tanggal_jam"]."' AND DATE_ADD('".$_POST["tanggal_jam"]."', INTERVAL ".$_POST["lama_waktu_pengerjaan"]." MINUTE) AND A.id<>'$id'");
    ...
    $data_waktu = $db->getFieldValue("soal_head A LEFT JOIN mapel AS B ON A.id_pelajaran=B.id", array("COUNT(*) AS TOTAL"), array("B.id_kelas" => $_POST["id_kelas"]), array(), 0, 0, "", " A.tanggal_jam<='".$_POST["tanggal_jam"]."' AND DATE_ADD(A.tanggal_jam, INTERVAL A.lama_waktu_pengerjaan MINUTE)>='".$_POST["tanggal_jam"]."'  AND A.id<>'$id'");
    if($data_waktu[0]["TOTAL"]>0) {
        $db->mysql_close();
        die(json_encode(array("success" => false, "message" => "Waktu pengerjaan soal tidak cocok / bentrok dengan ujian lain.")));
    }
    ...
    //UPDATE SOAL
    $db->mysql_update("soal_head", $_POST, array("id" => $_POST["id"]));
    
    $_SESSION["alert_success"] = "Edit soal berhasil.";
    $db->mysql_close();
