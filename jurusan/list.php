<?php
    ...
    $totalrecord = $db->getFieldValue("jurusan AS A LEFT JOIN sekolah AS B ON A.id_sekolah=B.id", array("COUNT(*) AS TOTAL"), array(),  array(), 0, 0, "", " 1 " . (strlen($query)>0?" AND (A.kode LIKE '%$query% OR A.nama LIKE '%$query%')":"").(strlen($id_sekolah)>0?" AND A.id_sekolah='$id_sekolah' ":""));
    ...
    $listdetail = $db->getFieldValue(
        "jurusan AS A LEFT JOIN sekolah AS B ON A.id_sekolah=B.id",  
        array(
            "A.id", 
            "B.nama_sekolah",
            "A.kode", 
    ...
    $smarty->assign("title", "Jurusan");
    $smarty->assign("sub_title", "Daftar Jurusan");
    ...
    $db->mysql_close();
    $smarty->display(BACK_END_TEMPLATE_PATH."/jurusan/list.html");    