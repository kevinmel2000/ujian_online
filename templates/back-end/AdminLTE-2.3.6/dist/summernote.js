...
(function (factory) {
  ...
  var async = (function () {
     ...
     */
    var createImage = function (sUrl, filename, desc) {
      return $.Deferred(function (deferred) {
        var $img = $('<img>');
        ...
        if(desc.length==0) {
            ...
            $img.one('load', function () {
              ...
              $img.off('load').detach();
              deferred.reject($figure);
            //}).css({
              //display: 'none'
            }).appendTo($figure).attr({
              'src': sUrl,
              'data-filename': filename
            });

            $( '<figcaption>'+desc+'</figcaption>' ).appendTo($figure);
            $figure.appendTo(document.body); 
        }
  ...
  var Editor = function (handler) {
     ...
     * @param {String} sUrl
     */
    this.insertImage = function (layoutInfo, $editable, sUrl, filename) {
        var $dialog = layoutInfo.dialog();
        var $imageDesc = $dialog.find('.note-image-desc');
        
        
      async.createImage(sUrl, filename, $imageDesc.val()).then(function ($image) {
        beforeCommand($editable);
        $image.css({
          display: '',
  ...
  var ImageDialog = function (handler) {
    ...
    this.show = function (layoutInfo) {
      ...
      this.showImageDialog($editable, $dialog).then(function (data) {
        ...
        if (typeof data === 'string') {
          // image url
          handler.invoke('editor.insertImage', layoutInfo, $editable, data);
        } else {
          // array of files
    ...
    this.showImageDialog = function ($editable, $dialog) {
      return $.Deferred(function (deferred) {
        ...
        var image_from_input = undefined;
        $imageDialog.one('shown.bs.modal', function () {
          ...
          $imageInput.replaceWith($imageInput.clone()
            .on('change', function () {
              ...
              image_from_input = this.files || this.value;
              toggleBtn($imageBtn, this.files || this.value);
            })
            .val('')
          );
          ...
          $imageBtn.click(function (event) {
            event.preventDefault();
            if(image_from_input) {
                deferred.resolve(image_from_input);
            } else {
        ...
        $.each(files, function (idx, file) {
          ...
          if (options.maximumImageFileSize && options.maximumImageFileSize < file.size) {
            ...
            async.readFileAsDataURL(file).then(function (sDataURL) {
              modules.editor.insertImage(layoutInfo, $editable, sDataURL, filename);
            }).fail(function () {
              bindCustomEvent($holder, callbacks, 'image.upload.error')(options.langInfo.image.maximumFileSizeError);
            });
          }
  ...
  var Renderer = function () {
    ...
    var tplDialog = function (className, title, body, footer) {
      return '<div class="' + className + ' modal" aria-hidden="false">' +
               '<br /><br /><br /><br /><div class="modal-dialog">' +
      ...
      image: function (lang, options) {
        ...
        var body = '<div class="form-group row note-group-select-from-files">' +
                   ...
                   '<div class="form-group row">' +
                     ...
                     '<input class="note-image-desc form-control col-md-12" type="text" />' +
                   '</div>';
