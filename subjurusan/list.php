<?php
    ...
    $totalrecord = $db->getFieldValue("subjurusan AS A LEFT JOIN jurusan AS B ON A.id_jurusan=B.id LEFT JOIN sekolah AS C ON B.id_sekolah=C.id", array("COUNT(*) AS TOTAL"), array("A.id_jurusan" => $id_jurusan),  array(), 0, 0, "", " 1 " . (strlen($query)>0?" AND (A.kode LIKE '%$query% OR A.nama LIKE '%$query%')":""));
    ...
    $listdetail = $db->getFieldValue(
        "subjurusan AS A LEFT JOIN jurusan AS B ON A.id_jurusan=B.id LEFT JOIN sekolah AS C ON B.id_sekolah=C.id",  
        array(
            "A.id", 
            "A.kode", 
            "A.nama"
        ), 
        array("A.id_jurusan" => $id_jurusan),  
    ...
    $smarty->assign("sub_title", "Daftar Sub Jurusan");
    ...
    $db->mysql_close();
    $smarty->display(BACK_END_TEMPLATE_PATH."/subjurusan/list.html");    